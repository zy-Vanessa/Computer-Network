...
std::vector<PacketTimer> packetTimers(10000);
...
void ShowPacket(Packet* pkt) {
    printf("[SYN:%d\tACK:%d\tFIN:%d\tEND:%d]SEQ:%d\tACK:%d\tcheckSUM:%d\tLEN:%d\n",
        pkt->head.flag & 0x1, pkt->head.flag >> 1 & 0x1, pkt->head.flag >> 2 & 0x1, pkt->head.flag >> 3 & 0x1,
        pkt->head.seq, pkt->head.ack, pkt->head.checkSum, pkt->head.bufSize);
}